...
class UserImageService {
    ...
    func loadRecordsData() {
        let fileURL = documentsURL.appendingPathComponent("recordsDataArray.json")

        do {
            let data = try Data(contentsOf: fileURL)
            recordsDataArray = try JSONDecoder().decode([RecordsDataModel].self, from: data)
        } catch {
    ...
    func rewriteUserImageArray() {
        var array: [UIImage?] = []
        for userRecord in recordsDataArray {
    ...
    func saveImageData(_ image: UIImage) {
        ...
        let fileURL = documentsURL.appendingPathComponent("selectedImage")
